...
WITHIN_KERNEL ftype
proxy_gauss_kronrod_quad(ftype a, ftype b, int N)
{
	// printf("Evaluation :: %d\n", N);
  // TODO: n adn w should be function arguments {{{
  // Nodes and weights for Gauss-Kronrod (7, 15)
  const ftype n15[15] = {-0.9914553711208126, -0.9491079123427585, -0.8648644233597691,
  ...
  const ftype n7[7] =  { -0.9491079123427585, -0.7415311855993944, -0.4058451513773972,
              0.0,
              0.4058451513773972,  0.7415311855993944,  0.9491079123427585 };
  
  const ftype w15[15] = { 0.0229353220105292,  0.0630920926299785,  0.1047900103222502,
  ...
  const ftype w7[7] =   { 0.1294849661688697,  0.2797053914892767,  0.3818300505051189,
              0.4179591836734694,
  ...
  if ( (Q7>1e300) || (Q15>1e300) )
	...
	{
      return proxy_gauss_kronrod_quad((a+b)/2, b, N+2) + proxy_gauss_kronrod_quad(a, (a+b)/2, 0);
	}
}


WITHIN_KERNEL ftype
quadgk(ftype a, ftype b)
{
  if (a == b)
	{
		return 0;
...
KERNEL void
kernel_quadgk(GLOBAL_MEM ftype *answer, ftype a, ftype b)
{
